...
final class ViewingConditions {
    ...
    static ViewingConditions make(float[] fArr, float f, float f2, float f3, boolean z) {
        ...
        float f18 = fArr7[1];
        return new ViewingConditions(yFromLStar, ((f17 * 2.0f) + ((f18 * 400.0f) / (f18 + 27.13f)) + (((400.0f * pow2) / (pow2 + 27.13f)) * 0.05f)) * pow, pow, pow, lerp, f12, fArr6, cbrt, (float) Math.pow(cbrt, 0.25d), sqrt);
    }
}
