...
public final class TemperatureCache {
    ...
    public Hct getComplement() {
        ...
        for (double d3 = 0.0d; d3 <= 360.0d; d3 += 1.0d) {
            ...
            if (isBetween(sanitizeDegreesDouble, d, hue)) {
                Hct hct4 = getHctsByHue().get((int) Math.round(sanitizeDegreesDouble));
                double abs = Math.abs(relativeTemperature - ((getTempsByHct().get(hct4).doubleValue() - doubleValue) / doubleValue2));
                if (abs < d2) {
                    hct3 = hct4;
                    d2 = abs;
                }
        ...
        this.precomputedComplement = hct3;
        return hct3;
    }
    ...
    /* synthetic */ Double m80xb7a75f35(Hct hct) {
