...
public class ReactScrollViewManager extends ViewGroupManager<ReactScrollView> implements ReactScrollViewCommandHelper.ScrollCommandHandler<ReactScrollView> {
    ...
    public void setContentOffset(ReactScrollView reactScrollView, ReadableMap readableMap) {
        if (readableMap != null) {
            reactScrollView.scrollTo((int) PixelUtil.toPixelFromDIP(readableMap.hasKey(ViewHierarchyNode.JsonKeys.X) ? readableMap.getDouble(ViewHierarchyNode.JsonKeys.X) : 0.0d), (int) PixelUtil.toPixelFromDIP(readableMap.hasKey(ViewHierarchyNode.JsonKeys.Y) ? readableMap.getDouble(ViewHierarchyNode.JsonKeys.Y) : 0.0d));
        } else {
    ...
    public Object updateState(ReactScrollView reactScrollView, ReactStylesDiffMap reactStylesDiffMap, StateWrapper stateWrapper) {
        reactScrollView.getMFabricViewStateManager().setStateWrapper(stateWrapper);
        return null;
    }
