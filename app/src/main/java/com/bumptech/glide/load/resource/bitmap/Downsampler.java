...
public final class Downsampler {
        ...
        if ((options.inSampleSize == 1 || z5) && shouldUsePool(imageType)) {
            if (i7 < 0 || i8 < 0 || !z2 || !z5) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    Log.v(TAG, "Calculated target [" + round + ViewHierarchyNode.JsonKeys.X + round2 + "] for source [" + i7 + ViewHierarchyNode.JsonKeys.X + i8 + "], sampleSize: " + i9 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                }
            } else {
                round = i4;
                round2 = i5;
    ...
    */
    ...
    private static String getBitmapString(Bitmap bitmap) {
        ...
        return "[" + bitmap.getWidth() + ViewHierarchyNode.JsonKeys.X + bitmap.getHeight() + "] " + bitmap.getConfig() + str;
    }
