...
public class RequestBuilder<TranscodeType> extends BaseRequestOptions<RequestBuilder<TranscodeType>> implements Cloneable, ModelTypes<RequestBuilder<TranscodeType>> {
    ...
    public RequestBuilder<TranscodeType> transition(TransitionOptions<?, ? super TranscodeType> transitionOptions) {
        if (isAutoCloneEnabled()) {
            return mo40clone().transition(transitionOptions);
        }
    ...
    public RequestBuilder<TranscodeType> listener(RequestListener<TranscodeType> requestListener) {
        if (isAutoCloneEnabled()) {
            return mo40clone().listener(requestListener);
        }
    ...
    public RequestBuilder<TranscodeType> addListener(RequestListener<TranscodeType> requestListener) {
        if (isAutoCloneEnabled()) {
            return mo40clone().addListener(requestListener);
        }
    ...
    public RequestBuilder<TranscodeType> error(RequestBuilder<TranscodeType> requestBuilder) {
        if (isAutoCloneEnabled()) {
            return mo40clone().error((RequestBuilder) requestBuilder);
        }
    ...
    private RequestBuilder<TranscodeType> cloneWithNullErrorAndThumbnail() {
        return mo40clone().error((RequestBuilder) null).thumbnail((RequestBuilder) null);
    }

    public RequestBuilder<TranscodeType> thumbnail(RequestBuilder<TranscodeType> requestBuilder) {
        if (isAutoCloneEnabled()) {
            return mo40clone().thumbnail(requestBuilder);
        }
    ...
    private RequestBuilder<TranscodeType> loadGeneric(Object obj) {
        if (isAutoCloneEnabled()) {
            return mo40clone().loadGeneric(obj);
        }
        this.model = obj;
    ...
    public RequestBuilder<TranscodeType> mo40clone() {
        RequestBuilder<TranscodeType> requestBuilder = (RequestBuilder) super.mo40clone();
        requestBuilder.transitionOptions = (TransitionOptions<?, ? super TranscodeType>) requestBuilder.transitionOptions.m41clone();
        ...
        if (requestBuilder2 != null) {
            requestBuilder.thumbnailBuilder = requestBuilder2.mo40clone();
        }
        RequestBuilder<TranscodeType> requestBuilder3 = requestBuilder.errorBuilder;
        if (requestBuilder3 != null) {
            requestBuilder.errorBuilder = requestBuilder3.mo40clone();
        }
    ...
    public ViewTarget<ImageView, TranscodeType> into(ImageView imageView) {
        ...
        if (!isTransformationSet() && isTransformationAllowed() && imageView.getScaleType() != null) {
            switch (AnonymousClass1.$SwitchMap$android$widget$ImageView$ScaleType[imageView.getScaleType().ordinal()]) {
                case 1:
                    requestBuilder = mo40clone().optionalCenterCrop();
                    break;
                case 2:
                    requestBuilder = mo40clone().optionalCenterInside();
                    break;
                case 3:
                case 4:
                case 5:
                    requestBuilder = mo40clone().optionalFitCenter();
                    break;
                case 6:
                    requestBuilder = mo40clone().optionalCenterInside();
                    break;
