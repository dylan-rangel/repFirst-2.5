...
public class GestureHandler<ConcreteGestureHandlerT extends GestureHandler<ConcreteGestureHandlerT>> {
    ...
    public static final void m181setEnabled$lambda3$lambda2(GestureHandler this_applySelf) {
    ...
    public final void prepare(View view, GestureHandlerOrchestrator orchestrator) {
        ...
        Window window = getWindow(view == null ? null : view.getContext());
    ...
    */
    ...
    private final void dispatchTouchMoveEvent(MotionEvent event) {
        ...
        while (i < pointerCount) {
            int i3 = i + 1;
            PointerData pointerData = this.trackedPointers[event.getPointerId(i)];
            if (pointerData != null) {
                if (pointerData.getX() == event.getX(i)) {
                    if (pointerData.getY() == event.getY(i)) {
                    }
                }
                pointerData.setX(event.getX(i));
                pointerData.setY(event.getY(i));
                pointerData.setAbsoluteX((event.getX(i) + rawX) - this.windowOffset[0]);
                pointerData.setAbsoluteY((event.getY(i) + rawY) - this.windowOffset[1]);
                addChangedPointer(pointerData);
                i2++;
            }
            i = i3;
    ...
    public final boolean isWithinBounds(View view, float posX, float posY) {
        ...
        if (fArr == null) {
            ...
            r4 = companion.hitSlopSet(f3) ? 0.0f - f3 : 0.0f;
    ...
    protected final PointF transformPoint(PointF point) {
        ...
        GestureHandlerOrchestrator gestureHandlerOrchestrator = this.orchestrator;
        PointF transformPointToViewCoords = gestureHandlerOrchestrator == null ? null : gestureHandlerOrchestrator.transformPointToViewCoords(this.view, point);
        if (transformPointToViewCoords != null) {
    ...
    public final ConcreteGestureHandlerT setEnabled(boolean enabled) {
        ...
        if (concretegesturehandlert.getView() != null && concretegesturehandlert.getIsEnabled() != enabled) {
            UiThreadUtil.runOnUiThread(new Runnable() { // from class: com.swmansion.gesturehandler.core.GestureHandler$$ExternalSyntheticLambda0
                ...
                public final void run() {
                    GestureHandler.m181setEnabled$lambda3$lambda2(GestureHandler.this);
                }
            });
        }
